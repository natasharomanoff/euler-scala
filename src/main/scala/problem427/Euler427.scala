 ...
 * n-sequences
 * 
 * A sequence of integers S = {si} is called an n-sequence if it has n elements and each element si satisfies 1  sin. Thus there are nn distinct n-sequences in total.
